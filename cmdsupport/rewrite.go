...
func genFileMap(g inspect.Global, session inspect.Session, rootDir string, opts *GenRewriteOptions) map[string]*Content {
	...
	for _, pkgRes := range contents {
		...
		for _, fileRes := range pkgRes.Files {
			...
			nrewriteFile++
			backMap[cleanGoFsPath(destFsPath(fileRes.OrigFile))] = &cmd.Content{
...
func GenRewrite(args []string, rootDir string, opts *GenRewriteOptions) (res *GenRewriteResult) {
	...
	pkgRw := inspect_old.NewMockRewritter(opts.RewriteOptions)
	return cmd.GenRewriteV2(args, rootDir, rw, pkgRw, opts)
}
...
func GenRewriteInit(args []string, rootDir string, opts *GenRewriteOptions) (res *GenRewriteResult) {
	...
	pkgRw := inspect_old.NewInitRewritter()
	return cmd.GenRewriteV2(args, rootDir, rw, pkgRw, opts)
}
